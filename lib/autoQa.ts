...
Be specific and balanced, not just critical: when the agent did something well, say so explicitly in a few words (e.g. "Good job -- stayed polite and patient throughout."). When the agent fell short, name the specific deviation in one plain sentence (e.g. "Never confirmed the customer's identity before discussing their account.").

CAMBODIAN CULTURAL CONTEXT for any item about using the customer's name or personalizing the greeting: addressing the customer with a respectful Khmer term like "bong" (បង) IS ALREADY the courtesy this kind of item is checking for -- it is standard, expected politeness in Cambodian customer service, not a shortcut or an omission. Do not mark such an item "N" just because the agent said "bong" instead of the customer's literal name; treat "bong" (or an equivalent respectful term of address) as satisfying it.
